...
| A7 | met | `install.sh` behavioral tests (`src/installScript.test.ts`, 5 cases incl. exit 2 on unknown installer); docs sections listed in SPEC P4; changeset `minor`; both smokes confined to `mkdtemp` roots (no writes to `~/.bun`, `~/.pi-web`, or live services). CI step for `smoke:bun-install` added but **never executed** — Actions has no recorded runs on this fork. |
