...
- [x] CI: `smoke:bun-install` added to the package-smoke job on Linux behind `oven-sh/setup-bun@v2` (`bun-version: latest`, because the runner image's copy is not guaranteed to provide `Bun.Terminal`). **No execution evidence anywhere:** `origin` is a fork whose Actions never ran (0 workflow runs recorded for the repository, workflow `state=active`, no check runs on the PR head SHA), so the step is inert until Actions is enabled.
...
- **Still not verified:** macOS launchd + brew-node preflight (no macOS host, §7.1) and the CI job itself — Actions has never run on this fork (§7.3), so neither smoke has runner evidence.
